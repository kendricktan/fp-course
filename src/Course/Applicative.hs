...
sequence = foldRight (lift2 (:.)) (return Nil)
...
replicateA i s = sequence (replicate i s)
...
filtering f = foldRight (\x -> lift2 (\b -> if b then (:.) x else id) (f x)) (return Nil)
